...
Interactive Evolutionay Computation (IEC) algorithms. A graph %specify
...
systems have an inherent drawback arising from the very nature of 
the algorithm, namely, the human fatigue caused by the interaction. % Can it be quantified? Maybe a reference to NodIO? - JJ
...
the EvoSpace-Interactive (ES-I) %citation
...
In order to do this, individuals about individuals %what? Maybe data
...
but it can be modified to handle other platforms. %A hint on how
...
Another example is the sharing action, that can occur when %Something
% missing here - JJ
