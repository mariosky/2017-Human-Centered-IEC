...
\begin{itemize}
<<<<<<< HEAD
  \item Individual-Individual. The relationships between individuals in the
=======
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth,height=8cm,keepaspectratio]{img/framework.png}
  
  \caption{Human-Centerd Framework.}
...
\begin{figure*}[h!tbp]
  ...
  \caption{User-Centered Framework.} %Maybe a bit of more explanation,
                                %showing the relationships and what
                                %they mean.
  \label{fig:uc_framework}       
\end{figure*}
%
...
\begin{itemize}
  \item Interactive Evolutionary Computational system. 
...
\item Users. This nodes represent the human users of the system and have 
the attributes: {\tt id} a unique identifier, {\tt name}  and {\tt created}
specifing the  creation date (time stamp) of the node. 

\item Individual. This type of node has the following attributes: 
{\tt id}, {\tt chromosome}  and {\tt views} this is 
the amount that users have seen or interacted with this individual and
 {\tt created}.
\item Collection. This type of nodes represent the collections belonging
to users, their attributes are {\tt id}, 
{\tt name}, {\tt created} and {\tt is_private} indicating if this collection
contents are shared with others. 
\end{itemize}

\subsection{Relationships Type}
%%
%% Explain relations

The {\tt likes} relation describes the interaction between a User and
an Individual in which a rating value is assigned. In the current application
a user is allowed to rate an Individual multiple times, so the date of the 
interactions is also of interest.
...
The relation {\tt knows} connects two users that know each other. This relation
is "unique" meaning that it can not be repeted between a pair of nodes.  

The relation {\tt parent} describes what Individual is the parent of a new
Individual and is also unique.  


The relation {\tt has} describes an ownership relation between users and
those collections they own. Also a collection {\tt has}  individuals stored
in them. 

In figure \ref{fig:Has_edge} the edge {\tt has} attributes and the relationship between certain nodes $user$, $individual$ and $collection$ are shown.
...
\bibliographystyle{IEEEtran}
\bibliography{evospace-i,volunteer}
