...
\begin{itemize}
  ...
  individuals according to users
...
evolutionary process in given Web-based IEC application.  This framework is 
presented in figure \ref{fig:uc_framework}. The current model is tailored 
...
The user centered framework consists of three high level components:

\begin{itemize}
  \item Interactive Evolutionary Computational system.  
  This is the real world system that we are going to model, it consist of
  human users and their interaction with one or more individuals from
  the population, this could be through a device, user interface or
  by directly interacting with real world objects generated as part of the algorithm.  

  \item IEC Graph.
  This is the model proposed in this work: A graph database modeling
  the IEC system. The  main components are the social network of users and the population. This database is the back-end of the system, is used by the EA and
  the user interface. 

  \item Evolutionary Algorithm. 
  This algorithm queries the graph model, and then updates those nodes that 
