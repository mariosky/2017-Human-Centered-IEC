...
which is to increase engagement of the users. % Adding this connecting
                                % to paper's objetives - JJ
                                % Thanks yes -Mario
...
phenotypes and a single phenotype could be shared by many collections. % This is why they are not attributes - Mario 
\end{itemize}

% Add something at the end linking with the next paragraph
% Done - Mario
The interaction between these entities are represented by the following relationships 
between nodes:
...
% Deleting the restriction, I think is not important and can be seen as a graph.
...
phenotype.  % Still think that user and individual is confusing - JJ
            ...
            % I was thinking about calling them Solutions or Candidate Solutions,
...
\ref{fig:top-ranked-participation} with a blue line. % Ranked by... - JJ
...
\begin{figure*}[!t]
    ...
    \caption{Top ranked number phenotypes rated by each user using a
     logarithmic scale.} % Explain a little more and label axes. The
...
B and GG (shown in figures \ref{fig:B-network} and \ref{fig:GG-network} respectively) additional
metrics and patterns are available when they are treated as a
network. %Please read the whole sentence and maybe rewrite - JJ
