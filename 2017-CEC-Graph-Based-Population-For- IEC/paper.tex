...
\title{Exploiting the Social Graph: Increasing Engagement in a
...
\author{\IEEEauthorblockN{Mario Garc\'ia-Valdez, Jos\'e Christian Romero Hern\'andez, Alejandra Mancilla}
...
one of the techniques employed by Evolutionary Art
researchers. It can be implemented as a web application to lower
...
volunteers interacting with a population also consisting of a network 
of phenotypes. The use of a graph model is proposed as a practical 
...
providing both conceptual and implementation details of the graph model 
as it is  applied in the implementation of an IEC system. Our
experiments show that the data model can be successfully used to implement 
a gamification technique developed to increase users' engagement, which
implies that this technique can be successfully used to 
decrease user fatigue and thus increase the {\em performance} of the
interactive system.
\end{abstract}
...
%basic subjectâ€“-predicate-â€“object expressions
...
(the source code is available in GitHub at \url{jcromerohdz/evoDrawing_gaming}).
...
\begin{figure*}[!t]
    \centering
        \includegraphics[width=5.5in]{img/graphKnows.png}
    \caption{User graph showing the KNOWS relationship (B deployment).}
    \label{fig:B-network}
\end{figure*}
\begin{figure*}[!t]
    \centering
        \includegraphics[width=5.5in]{img/comparison.png}
    \caption{Users ranked by the number of phenotypes they rated (X axis) vs. the number of phenotypes evaluated using a logarithmic scale (Y axis). } % Explain a little more and label axes. The
