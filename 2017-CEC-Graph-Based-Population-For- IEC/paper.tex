...
also depend on the previous activites: they have the same tag?, they were shared by
similar users? a leader of opinion liked both? they are stored in the same collection?.
That knowledge could also be applied in choosing which phenotypes present to a
...
interactions. Graph based models are currently used in social networks to keep track
...
complex relationships for instance: ``Jhon and Ann eating breakfast at Tony's''. 
Other example is the W3C Activity Streams 2.0 specification used for representing activities 
common in social web applications.  
